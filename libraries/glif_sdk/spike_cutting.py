'''came from allensdk.internal.model.glif.spike_cutting import calc_spike_cut_and_v_reset_via_expvar_residuals'''

from find_spikes import align_and_cut_spikes, ALIGN_CUT_WINDOW
import numpy as np
...
def calc_spike_cut_and_v_reset_via_expvar_residuals(all_current_list, 
    ...
    '''This function calculates where the spike should be cut based on explained variance.  
        ...
        inputs: 
            all_current_list: list of current 
                list of current traces injected into neuron)
            all_voltage_list: list of voltages 
                (list of voltage trace)
    ...
    #convert all_thresholdInd to indicies
    all_thresholdInd=np.array(all_thresholdInd, dtype=int)
